...
package br.com.softplan.ocr.scanner;
 ...
 */
public abstract class OCRScannerImage {
     ...
     * @throws OCRException 
     */
    public final void scanImage(OCRImage ocrImage, OCRScannerListenerImage listener) throws OCRException {
    	this.scanImage(ocrImage, listener, 0, 0, ocrImage.getImageScale().getWidth(), ocrImage.getImageScale().getHeight());
    }
     ...
     * @throws OCRException 
     */
    public final void scanImage(OCRImage ocrImage, OCRScannerListenerImage listener, int blockX1, int blockY1, int blockX2, int blockY2) throws OCRException {
        int[] pixels = ocrImage.getBinaries();
        int w = ocrImage.getImageScale().getWidth();
        int h = ocrImage.getImageScale().getHeight();
        ...
        listener.beginScanImage(ocrImage);
        
        // First build list of rows of text.
        ArrayList<Integer> rowsOfText = new ArrayList<Integer>();
        int y1 = 0;
        for (int y = blockY1; y < blockY2; y++) {
            ...
            if (isWhiteSpace) {
                if (!whiteLine) {
                    whiteLine = true;
                    rowsOfText.add(new Integer(y1));
                    rowsOfText.add(new Integer(y));
                }
            } else {
                if (whiteLine) {
                    whiteLine = false;
        ...
        if (!whiteLine) {
            rowsOfText.add(new Integer(y1));
            rowsOfText.add(new Integer(blockY2));
        }
        ...
        // underscores.
        for (int i = 0; (i + 4) <= rowsOfText.size(); i += 2) {
            int bY0 = (rowsOfText.get(i)).intValue();
            int bY1 = (rowsOfText.get(i + 1)).intValue();
            int bY2 = (rowsOfText.get(i + 2)).intValue();
            int bY3 = (rowsOfText.get(i + 3)).intValue();
            int row0H = bY1 - bY0;
            int whiteH = bY2 - bY1;
            int row1H = bY3 - bY2;
            if (((row1H <= (int) ((float) row0H * shortRowFraction))
                    ...
                    || (whiteH < 6))) {
                rowsOfText.remove(i + 2);
                rowsOfText.remove(i + 1);
                i -= 2;
            }
        }
        if (rowsOfText.size() == 0) {
            rowsOfText.add(new Integer(blockY1));
            rowsOfText.add(new Integer(blockY2));
        }
        // Process the rows.
        for (int i = 0; (i + 1) < rowsOfText.size(); i += 2) {
            int bY1 = (rowsOfText.get(i)).intValue();
            int bY2 = (rowsOfText.get(i + 1)).intValue();

            this.processRow(ocrImage, listener, pixels, w, h, blockX1, bY1, blockX2, bY2);
        }
        listener.endScanImage(ocrImage);
    }
     ...
     * @throws OCRException
     */
    private void processRow(OCRImage ocrImage, OCRScannerListenerImage listener, int[] pixels, int w, int h, int x1, int y1, int x2, int y2) throws OCRException {
        listener.beginRow(ocrImage, y1, y2);
        int rowHeight = y2 - y1;
        ...
        for (int i = 0; (i + 1) < al.size(); i += 2) {
            if (i >= 2) {
                ...
                while ((cx2 - cx1) >= minSpaceWidth) {
                    ...
                    listener.processSpace(ocrImage, cx1, y1, sx2, y2);
                    cx1 += minSpaceWidth;
                }
            }
            ...
            if (cy1 >= cy2) {
                ...
                listener.processSpace(ocrImage, cx1, y1, cx2, y2);
            } else {
                listener.processCharacter(ocrImage, cx1, cy1, cx2, cy2, y1, y2);
            }
        }
        listener.endRow(ocrImage, y1, y2);
    }
}