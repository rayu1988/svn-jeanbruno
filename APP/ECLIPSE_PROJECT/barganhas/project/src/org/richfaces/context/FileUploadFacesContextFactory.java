...
public class FileUploadFacesContextFactory extends FacesContextFactory implements FacesWrapper<FacesContextFactory> {
	...
	private static final java.util.logging.Logger gaeLogger = java.util.logging.Logger.getLogger(FileUploadFacesContextFactory.class.getCanonicalName());
...
//        }
    ...
    private HttpServletRequest wrapMultipartRequestServlet25(ServletContext servletContext, HttpServletRequest request,
        ...
        gaeLogger.log(Level.INFO, "Getting the param max request size to file upload, that is: " + maxRequestSize);

        gaeLogger.log(Level.INFO, "Checking if the contentLength current is greater than maxRequestSize.");
        if (maxRequestSize == 0 || contentLength <= maxRequestSize) {
        	gaeLogger.log(Level.INFO, "NO, the contentLength current is not greater than maxRequestSize.");
        	
            boolean createTempFiles = isCreateTempFiles(servletContext);
            gaeLogger.log(Level.INFO, "Getting the param createTempFiles, that is: " + createTempFiles);
            
            String tempFilesDirectory = getTempFilesDirectory(servletContext);
            gaeLogger.log(Level.INFO, "Getting the temp files directory, that is: " + tempFilesDirectory);

            gaeLogger.log(Level.INFO, "(before) Instantiating a new MultipartRequestParser.");
            MultipartRequestParser requestParser = new MultipartRequestParser(request, createTempFiles, tempFilesDirectory,
                progressControl);
            gaeLogger.log(Level.INFO, "(after) Instantiating a new MultipartRequestParser.");

            gaeLogger.log(Level.INFO, "(before) Instantiating a new MultipartRequest25.");
            multipartRequest = new MultipartRequest25(request, uploadId, progressControl, requestParser);
            gaeLogger.log(Level.INFO, "(after) Instantiating a new MultipartRequest25.");
        } else {
        	gaeLogger.log(Level.INFO, "YES, the contentLength current is greater than maxRequestSize, let's throw an Exception Size Exceeded.");
            multipartRequest = new MultipartRequestSizeExceeded(request, uploadId, progressControl);
        }

        gaeLogger.log(Level.INFO, "Setting the multipartRequest to request attribute scope that has as identifier: " + MultipartRequest.REQUEST_ATTRIBUTE_NAME);
        request.setAttribute(MultipartRequest.REQUEST_ATTRIBUTE_NAME, multipartRequest);

        gaeLogger.log(Level.INFO, "Returning the multipartRequest generated.");
